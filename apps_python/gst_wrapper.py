...
# How long free() waits for EOS before tearing the pipeline down anyway.
EOS_TIMEOUT_NS = 10 * Gst.SECOND
...
class GstPipe:
    ...
    def free(self):
        ...
        """
        # Wait for EOS in the sink pipeline, but never forever. The hardware
        # encoder does not always drain on EOS, and with an unbounded wait the
        # whole app used to sit here with every thread asleep. Ctrl+C cannot
        # break out of a blocking bus call, so the only way out was kill -9,
        # which is a dirty shutdown and wedges TIOVX until the next reboot. A
        # bounded wait ends the run cleanly instead.
        bus = self.sink_pipe.get_bus()
        msg = bus.timed_pop_filtered(
            EOS_TIMEOUT_NS, Gst.MessageType.ERROR | Gst.MessageType.EOS
        )
        if msg:
            ...
            print(
                "[WARN] No EOS within %d s, shutting the pipeline down anyway. "
                "A file sink may be left unfinalized; prefer .mkv over .mp4 "
                "for file outputs." % (EOS_TIMEOUT_NS // Gst.SECOND)
            )
...
def get_output_elements(output):
    ...
    if sink == "display":
        ...
        if output.encoding in ("mp4", "h264", "h264-rtp"):
            if (gst_element_map["h264enc"]["element"] == "v4l2h264enc"):
                property = {"extra-controls": Gst.Structure.from_string(enc_extra_ctrl)[0]}

            sink_elements += make_element(gst_element_map["h264enc"], property=property)

            h264parse_property = None
            if output.encoding == "h264":
                """
                config-interval=-1 re-sends SPS/PPS in band with every IDR
                frame. Without it the parameter sets are only emitted once at
                the start of the stream, so a receiver that joins the UDP
                MPEG-TS stream later never sees them and fails with
                "non-existing PPS 0 referenced" / "no frame!".

                Only needed on this branch: mp4mux stores the parameter sets in
                the container header, and rtph264pay signals them out of band
                through its own sprop-parameter-sets caps field.
                """
                h264parse_property = {"config-interval": -1}

            sink_elements += make_element("h264parse", property=h264parse_property)
            ...
            if output.encoding == "mp4":
                ...
                """
                MPEG-TS over UDP. Self describing on the wire, so VLC and a
                plain "udpsrc ! tsdemux ! ..." receiver can both play it with
                no SDP file. alignment=7 packs 7 TS packets (7*188 = 1316
                bytes) per buffer, which is the largest whole number of TS
                packets that fits in a standard 1500 byte MTU datagram.
                """
                property = {"alignment": 7}
                sink_elements += make_element("mpegtsmux", property=property)
            elif output.encoding == "h264-rtp":
                """
                RTP payloaded H264. config-interval=-1 re-sends SPS/PPS in band
                with every IDR, for the same reason as the MPEG-TS branch: a
                receiver that joins mid-stream must see the parameter sets or
                it decodes nothing at all. Without it they are sent once, at
                the very start of the stream, and are gone forever after that.
                Needs an SDP on the receiving side, or a GStreamer receiver
                with explicit caps on udpsrc.
                """
                property = {"config-interval": -1}
                sink_elements += make_element("rtph264pay", property=property)
        ...
        # Jpeg encoding
            ...
            print(
                ...
                "Supported: h264 (MPEG-TS over UDP), h264-rtp, mp4, jpeg."
                % output.encoding
            )
            sys.exit()
...
def get_post_proc_elements(flow):
    ...
    # Describes the frame post processing pushes into this appsrc.
    caps = "video/x-raw, format=NV12, width=%d, height=%d" % (
        flow.post_proc_width,
        flow.post_proc_height,
    )
        ...
        for s_index, s in enumerate(f.sub_flows):
            ...
            for index, o in enumerate(s.outputs):
                if o.mosaic:
                    ...
                    post processed frame dimension of this subflow
                    """
                    if o.width != s.post_proc_width or o.height != s.post_proc_height:
                        ...
                        if s.flow.is_multi_scaler:
                            ...
                            scaler_element = []
                            if (
                                s.post_proc_width / o.width > 4
                                or s.post_proc_height / o.height > 4
                            ):
                                # width = max(s.post_proc_width//4, o.width)
                                # height = max(s.post_proc_height//4, o.height)
                                width = (s.post_proc_width + o.width) // 2
                                height = (s.post_proc_height + o.height) // 2
                                if width % 2 != 0:
                                    width += 1
                                if height % 2 != 0:
                                    height += 1
