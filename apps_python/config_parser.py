...
class SubFlow:
    ...
    def __init__(self, input, subflow_list, flow):
        ...
        self.enable_tracking = getattr(self.model, "enable_tracking", False)

        # Size of the frame the sensor path feeds to post processing.
        self.sensor_width = 0
        self.sensor_height = 0
        # Size of the frame post processing pushes back into the pipeline.
        # Post processing draws in place, tracking included, so this is always
        # the same as the sensor frame. Kept as its own pair of fields because
        # that is the size gst_wrapper and infer_pipe actually care about.
        self.post_proc_width = 0
        self.post_proc_height = 0
        self.mosaic_info = []
        ...
        for i in range(len(self.outputs)):
            ...
            self.post_proc_width = max(self.post_proc_width, self.width)
            self.post_proc_height = max(self.post_proc_height, self.height)
